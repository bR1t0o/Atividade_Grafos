...
#Professor, não tive a oportunidade de aplicar todos os métodos em JAVA ainda (linguagem que possuo mais conhecimento). 
# Então utilizei o seu código em python. E, acredito que seja por falta de pratica na linguagem, não soube implementar recursão para o método discutido em sala de aula.
# Enquanto tentava resolver o problema tive essa ideia de algoritmo. Apesar de parecer ineficiente, funcionou. Por isso a discrepância entre os métodos discutidos e o que eu fiz.
# Mando em anexo o rascunho que me fez pensar nesse algoritmo. 
# Obrigado!
class Grafo(ABC):
    @abstractmethod
...
class GrafoDenso(Grafo):
    ...
    def color_Graph(self):
        ...
        for d in range(len(self.matriz)):
           lista_cores.append( d+1)     


        ##print(lista_cores)
        #colors.update({self.labels[0]:lista_cores[0]}) 
        
       
        for i in range(len(self.matriz)):
          ## colors.update(vertices_labels[i]:"Red")
          vertice_atual = self.labels[i]

          cor_adjacente = []
          for j in range(len(self.matriz[i])):
            if self.matriz[i][j] == 1:
                vertice_adjacente = self.labels[j]
                if vertice_adjacente in colors:
                    cor_adjacente.append(colors[vertice_adjacente])
                    
                for cor in lista_cores:
                    if (cor not in cor_adjacente):
                        colors[vertice_atual] = cor
                        break
                
        for n in range(len(self.matriz)):
            
            print(self.labels[n],colors[self.labels[n]]) 
    
        print("Número de cores utilizadas: ",len(set(colors.values()))) #set pra num contar duplicada
